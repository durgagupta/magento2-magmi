...
//      
        foreach ($conddata as $c) {
        	
        }
        $this->log("{$sql}---","warning");
        $this->log("PID {$pid}---","warning");
...
//      $this->dolink($pid, "IN (" . $this->arr2values($skulist) . ")", $skulist);
        ...
        $this->dolink($pid, "IN (" . implode(",",$inArray) . ")", array());
    }
    ...
    public function processItemAfterId(&$item, $params = null)
    {
        ...
        switch ($matchmode) {
            case "none":
                break;
            case "auto":
            	$this->log($item["simples_skus"]."---auto","warning");
                // destroy old associations
                $this->autoLink($pid);
                $this->updSimpleVisibility($pid);
                break;
            case "cursimples":
            	$this->log($item["simples_skus"]."---cursimples","warning");
                $this->fixedLink($pid, $this->_currentsimples);
                $this->updSimpleVisibility($pid);

                break;
            case "fixed":
            	$this->log($item["simples_skus"],"warning");
            	
            	$this->log($item["simples_skus"]."---fixed","warning");
